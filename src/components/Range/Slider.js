...
import "./Range.scss";
import sliderTriangleImg from "./img/triangle.svg";
...
const Slider = (props) => {
  ...
  const markers_PositionsX = useMemo(() => {
    ...
    for (let i = 0; i <= markers; ++i) {
      const relPosition = i * (trackLength / markers);
  ...
  const sliderChangeHandler = useCallback((position) => {
    const updatedValue = Math.round(min + position / stepWidth);
    if (updatedValue !== value && updatedValue % stepSize === 0) {
      setHandler_position(
        markers_PositionsX.find((x) => x.marker_value === updatedValue).position
      );
    ...
    return (
      ...
      >
        ...
        <>
          ...
          >
            <img src={sliderTriangleImg} alt="slider"></img>
          </div>
        </>
      </div>
    );
            ...
            style={{
              ...
              visibility:
                ...
                (showMarkers === "inactive" && !isActive)
                  ? "visible"
                  : "hidden",
              ...(!vertical ? { top: "-7.5px" } : { width: markersSize }),
            }}
          >
            {markers_PositionsX.map(({ position, marker_value }) => {
              return (
                <div
                  key={position}
                  style={{
                    ...
                    ...(!vertical
                      ? {
                          left: position,
                          background:
                            marker_value === value ? "#ffbc36" : "#898989",
                          width: marker_value === value ? "7px" : "3px",
                          height: marker_value === value ? "7px" : "3px",
